...
export default function CodeModification({ codebasePath, apiKey, onOperationComplete }: CodeModificationProps) {
  ...
  const handleOperation = async (operation: string) => {
    ...
    try {
      ...
      switch (operation) {
        case 'edit':
          result = await sdkService?.editFile(filePath, content)
          break
        case 'create':
          result = await sdkService?.createFile(filePath, content)
          break
        case 'delete':
          result = await sdkService?.deleteFile(filePath)
          break
        case 'analyze':
          result = await sdkService?.analyzeCodbase()
          break
        default:
      ...
      toast({
        title: "Success",
        description: result.message || `Operation ${operation} completed successfully`,
      })
    } catch (error) {
